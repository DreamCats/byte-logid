...
byte-logid 是字节跳动内部的日志查询 CLI 工具，通过 Log ID（Trace ID）在内部日志服务（StreamLog）中检索分布式调用链路日志，帮助开发者快速排查问题。

当前 v0.1.x 版本存在以下可优化点：

- **命令结构冗余**：必须使用 `byte-logid query <LOGID>` 子命令形式，日常使用中 `query` 是多余的
- **认证逻辑内置**：自己管理 `.env` 中的 `CAS_SESSION`，与 byte-auth 统一认证工具重复
- **过滤规则硬编码**：消息净化规则写死在代码中，用户无法灵活调整
- **缺少关键词筛选**：返回数据量大时，无法按关键词快速定位关键信息

v2.0 的目标是解决上述问题，使 byte-logid 更简洁、更灵活，特别是对 AI Agent 场景更友好。
...
**去掉 `query` 子命令**，byte-logid 直接接受位置参数作为 Log ID：

```bash
# v1.x（旧）
byte-logid query <LOGID> --region us

# v2.0（新）
byte-logid <LOGID> --region us
```
...
byte-logid update [--check] [--force]
byte-logid config [SUBCOMMAND]
byte-logid --version
byte-logid --help
```
...
| 模式 | 说明 | 使用场景 |
|------|------|----------|
| 自动获取 | byte-logid 内部调用 `byte-auth token --region <region> --raw` | 默认模式，用户无感 |
...
**将硬编码的过滤规则移到用户级配置文件**：`~/.config/byte-logid/filters.json`
...
- **首次运行**：如果 `~/.config/byte-logid/filters.json` 不存在，自动生成包含默认规则的配置文件
- **配置文件为唯一来源**：代码中不再硬编码默认规则，所有规则从配置文件读取
- **用户可编辑**：直接编辑 JSON 文件增删规则
- **命令行管理**：通过 `byte-logid config filter` 子命令管理规则
...
# 添加过滤规则
byte-logid config filter add '<正则表达式>'
...
```bash
# 只看包含 ForLive 的日志
byte-logid <LOGID> -r us -k ForLive

# 多个关键词（OR 关系，包含任一即保留）
byte-logid <LOGID> -r us -k ForLive -k ErrorCode

# 与 PSM 过滤组合使用
byte-logid <LOGID> -r us -p my.service -k ForLive
```
...
```bash
byte-logid <LOGID> -r us -p service.a -p service.b
```
